...
func (wac *Conn) Login(qrChan chan<- string) (Session, error) {
	...
	//oldVersion=8691
	login := []interface{}{"admin", "init", []int{0, 4, 1307}, []string{wac.longClientName, wac.shortClientName}, session.ClientId, true}
...
func (wac *Conn) Restore() error {
	...
	//admin init
	init := []interface{}{"admin", "init", []int{0, 4, 1307}, []string{wac.longClientName, wac.shortClientName}, wac.session.ClientId, true}
